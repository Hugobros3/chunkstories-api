...
/** Helper interface regrouping all the look & feel specific traits of a
 * World */
public interface WorldEnvironment {

	/** How 'wet' ( reflective ) should the world be rendered ? Within [0:1]. Can
	 * vary depending on camera position. */
	public abstract float getWorldWetness(Vector3dc cameraPosition);

	/** Returns the texture used to colour the grass & other opaque textures
	 * depicting vegetation, this texture is mapped to 1-1 to the world
	 * boundaries */
	public abstract Texture2D getGrassTexture(RenderingInterface renderer);

	/** Returns the sunlight color. Be advised, the sunlight is added to the shadow
	 * light. */
	public abstract Vector3fc getSunlightColor(Vector3dc cameraPosition);

	/** Returns the shadow ('ambient') color. Be advised, this is multiplied with
	 * the voxel sunlight data, so a pitch-black cave will not pick up any of this
	 * color. */
