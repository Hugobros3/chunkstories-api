...
public interface ChunkStoriesLogger_ {
	...
	public enum LogType {
		...
		, CLIENT
		/** Has to do with client logic */
		, SERVER
		/** Has to do with server logic */
		, @Deprecated
		GAMEMODE
		/** Deprecated */
		, CONTENT_LOADING
		/**
		 * Has to do with loading content ( ie: downloading mods, loading item
		 * definitions etc )
		 */
		, PLUGIN
		/** Inside a plugin or foreign code in general */
		, WORLD_TICKING
		/** On the main world logic loop */
		, WORLD_GENERATION
		/** While generating map data */
		, WORLD_STREAMING
		/** Streaming, both local and remote */
